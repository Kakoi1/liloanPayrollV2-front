      ...
      <div class="container-fluid px-6 py-4">
        <div class="flex flex-wrap items-center justify-between mb-2">
          <div class="sm:col-span-6">
            <h1 class="text-2xl font-bold text-gray-800 m-0">Payment Methods</h1>
          </div>
          <div class="sm:col-span-6">
            <ol class="flex space-x-2 text-sm">
              ...
              <li class="breadcrumb-item text-gray-500">Payment Method</li>
            </ol>
          </div>
        </div>
      </div>
        ...
        <div class="w-full">
          <div class="bg-white rounded-lg shadow-md">
            ...
            <div class="px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-700 rounded-t-lg flex justify-between items-center">
              <h5 class="text-white font-semibold text-lg">Payment Methods Table</h5>
...
const fetchCompensations = async () => {
  try {
    ...
    const response = await api.post('/payment-method/list', formData)
    
    if (response.data && !response.data.error) {
      // Add isEditing flag to each item
      data.value = (response.data.pm_data || []).map(item => ({
        ...item,
...
const updateCompensation = async (index) => {
  ...
  try {
    const response = await api.post('/payment-method/edit', {
      id: item.id,
      name: item.name,
...
const confirmDelete = async (id) => {
  ...
  if (result.isConfirmed) {
    ...
    try {
      const response = await api.post('/payment-method/delete', {
        id: id
      })
